...
En este paso añadiremos el botón "Registrar Signos Vitales" al panel lateral de acciones rápidas del paciente.
...
* **Configurar las propiedades (Inspector -> Properties)**:
  - **Text**: Establezca el texto en `📊 Registrar Signos Vitales`.
...
* **Vincular el código (Inspector -> Code)**:
  - **fx:id**: Asigne el identificador `btnRegistrarSignos`.
  - **On Action**: Ingrese el método `#registrarSignos`.
* **Guardar**: Presione `Ctrl + S`.

---

## 3. Crear o Modificar el Diálogo de Signos Vitales

Diseñaremos una ventana de diálogo dedicada únicamente a capturar constantes vitales para el registro de signos vitales.

* **Abrir el archivo**: Abra `nuevo_signos_vitales_dialog.fxml` en Scene Builder.
...
* **Añadir el Título**:
  - Arrastre un control `Label` con el texto `REGISTRAR NUEVOS SIGNOS VITALES`.
...
* **Definir el controlador**: En la esquina inferior izquierda de Scene Builder, abra la pestaña **Controller** y en **Controller Class** escriba la ruta del controlador:  
  `com.clinica.aauca.controller.NuevoSignosVitalesDialogController`
